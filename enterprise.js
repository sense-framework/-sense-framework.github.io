(() => {
  ...
  function attentionList(ws,company) {
    const items=[];
    ...
    const openTickets=(company.tickets||[]).filter(item=>!['Resolved','Closed'].includes(item.status)).length;
    if(openTickets)items.push([`${openTickets} open support ticket${openTickets===1?'':'s'}`,'support']);
    ...
    return items.length?items.map(([label,view])=>`<button ${view==='support'?'data-open-public-support="1"':view==='applications'?`class="enterprise-nav" data-enterprise-view="applications"`:`data-existing-view="${view}"`}><span>${esc(label)}</span><b>→</b></button>`).join(''):empty('✓','Nothing requires attention');
  }
